...
locals {
  lambdaName = "${var.stage}-api-forum"
...
# GET
resource "aws_api_gateway_method" "forum_get" {
  rest_api_id = "${var.api_id}"
...
resource "aws_api_gateway_integration" "forum_get_lambda_integration" {
  rest_api_id = "${var.api_id}"
  resource_id = "${aws_api_gateway_resource.forum.id}"
  http_method = "${aws_api_gateway_method.forum_get.http_method}"
